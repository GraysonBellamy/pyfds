# About PyFDS

Learn more about the PyFDS project.

## Project Information

<div class="grid cards" markdown>

-   :material-history: **Changelog**

    ---

    Version history and release notes

    [:octicons-arrow-right-24: Changelog](changelog.md)

-   :material-scale-balance: **License**

    ---

    MIT License terms and conditions

    [:octicons-arrow-right-24: License](license.md)

-   :material-book-open-variant: **Citation**

    ---

    How to cite PyFDS in publications

    [:octicons-arrow-right-24: Citation](citation.md)

-   :material-hand-heart: **Acknowledgments**

    ---

    Contributors and supporters

    [:octicons-arrow-right-24: Acknowledgments](acknowledgments.md)

</div>

## Project Links

- :fontawesome-brands-github: [GitHub Repository](https://github.com/GraysonBellamy/pyfds)
- :fontawesome-brands-python: [PyPI Package](https://pypi.org/project/pyfds/)
- :material-file-document: [Documentation](https://pyfds.graysonbellamy.dev/)
- :material-bug: [Issue Tracker](https://github.com/GraysonBellamy/pyfds/issues)
- :material-forum: [Discussions](https://github.com/GraysonBellamy/pyfds/discussions)

## Quick Facts

| Attribute | Value |
|-----------|-------|
| **Author** | Grayson Bellamy |
| **License** | MIT |
| **Python** | 3.11+ |
| **Status** | Alpha (Active Development) |
| **Latest Version** | 0.1.0 |

## Contact

- **Email**: graysontbellamy@gmail.com
- **GitHub**: [@GraysonBellamy](https://github.com/GraysonBellamy)

## Related Projects

### FDS Ecosystem
- [FDS](https://pages.nist.gov/fds-smv/) - Fire Dynamics Simulator by NIST
- [Smokeview](https://pages.nist.gov/fds-smv/) - FDS visualization tool
- [BlenderFDS](https://github.com/firetools/blenderfds) - Blender interface for FDS

### Python Fire Modeling
- [fdsreader](https://github.com/FireDynamics/fdsreader) - Python FDS output file reader

## Contributing

We welcome contributions! See the [Contributing Guide](../development/contributing.md) for details.

---

[View Changelog →](changelog.md){ .md-button .md-button--primary }
