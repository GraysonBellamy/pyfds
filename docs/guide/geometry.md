...
```python
sim.add(Obstruction(ion(xb=Bounds3D.of(1.0, 2.0, 1.0, 2.0, 0.0, 0.5))
```
...
# West wall (low X)
sim.add(Obstruction(ion(xb=Bounds3D.of(0, wall_thickness, 0, 5, 0, 2.5), surf_id='WALL')

# East wall (high X)
sim.add(Obstruction(ion(xb=Bounds3D.of(5-wall_thickness, 5, 0, 5, 0, 2.5), surf_id='WALL')

# South wall (low Y)
sim.add(Obstruction(ion(xb=Bounds3D.of(0, 5, 0, wall_thickness, 0, 2.5), surf_id='WALL')

# North wall (high Y)
sim.add(Obstruction(ion(xb=Bounds3D.of(0, 5, 5-wall_thickness, 5, 0, 2.5), surf_id='WALL')
```
...
sim.add(Obstruction(ion(xb=Bounds3D.of(0, 5, 0, 5, 0, 0.01), surf_id='FLOOR')

# Ceiling (at top of domain)
sim.add(Obstruction(ion(xb=Bounds3D.of(0, 5, 0, 5, 2.49, 2.5), surf_id='CEILING')
```
...
sim.add(Obstruction(ion(xb=Bounds3D.of(2.0, 3.5, 1.5, 2.3, 0.0, 0.8), surf_id='WOOD')

# Chair: 0.5m × 0.5m × 0.5m
sim.add(Obstruction(ion(xb=Bounds3D.of(1.5, 2.0, 1.5, 2.0, 0.0, 0.5), surf_id='PLASTIC')

# Cabinet: 1m × 0.5m × 2m tall
sim.add(Obstruction(ion(xb=Bounds3D.of(0.2, 1.2, 4.5, 5.0, 0.0, 2.0), surf_id='METAL')
```
...
sim.add(Obstruction(ion(xb=Bounds3D.of(2.35, 2.65, 2.35, 2.65, 0.0, 2.5), surf_id='CONCRETE')

# Multiple columns in a row
for x in [1.0, 2.0, 3.0, 4.0]:
    sim.add(Obstruction(ion(xb=Bounds3D.of(x-0.15, x+0.15, 2.35, 2.65, 0.0, 2.5), surf_id='CONCRETE')
```
...
def create_bookshelf(sim, x, y, z, width, depth, height, shelves):
    ...
    # Back and sides
    sim.add(Obstruction(ion(xb=Bounds3D.of(x, x+width, y+depth-0.02, y+depth, z, z+height), surf_id='WOOD')
    sim.add(Obstruction(ion(xb=Bounds3D.of(x, x+0.02, y, y+depth, z, z+height), surf_id='WOOD')
    sim.add(Obstruction(ion(xb=Bounds3D.of(x+width-0.02, x+width, y, y+depth, z, z+height), surf_id='WOOD')
...
sim.add(Obstruction(ion(xb=Bounds3D.of(0, 1, 0, 1, 0, 1))

# Explicit surface
sim.add(Obstruction(ion(xb=Bounds3D.of(0, 1, 0, 1, 0, 1), surf_id='CONCRETE')
```
...
```python
# Create wall
sim.add(Obstruction(ion(xb=Bounds3D.of(5, 5.2, 0, 5, 0, 3), surf_id='WALL')

# Cut window in wall
sim.add(Obstruction(ion(xb=Bounds3D.of(5, 5.2, 1.5, 2.5, 1.5, 2.5), hole=True)
```
...
sim.add(Obstruction(ion(xb=Bounds3D.of(1.07, 2.13, 1.05, 1.98, 0.02, 0.47))
```
...
```python
# Bad - tiny 0.01m gap
sim.add(Obstruction(ion(xb=Bounds3D.of(0, 2.49, 0, 5, 0, 2.5))
sim.add(Obstruction(ion(xb=Bounds3D.of(2.51, 5, 0, 5, 0, 2.5))

# Good - touching or overlapping
sim.add(Obstruction(ion(xb=Bounds3D.of(0, 2.5, 0, 5, 0, 2.5))
sim.add(Obstruction(ion(xb=Bounds3D.of(2.5, 5, 0, 5, 0, 2.5))
```
...
sim.add(Obstruction(ion(xb=Bounds3D.of(4.8, 5, 0, 5, 0, 2.5), surf_id='WALL')

# ============================================================
# FURNITURE
# ============================================================
sim.add(Obstruction(ion(xb=Bounds3D.of(1, 2, 1, 2, 0, 0.8), surf_id='TABLE')
sim.add(Obstruction(ion(xb=Bounds3D.of(1, 1.5, 1, 1.5, 0, 0.5), surf_id='CHAIR')

# ============================================================
# FIRE SOURCES
# ============================================================
sim.add(Obstruction(ion(xb=Bounds3D.of(2.5, 3.5, 2.5, 3.5, 0, 0.1), surf_id='BURNER')
```
...
def add_rectangular_room(sim, xb, wall_thickness, surf_id='WALL'):
    ...
    # Four walls
    sim.add(Obstruction(ion(xb=Bounds3D.of(xmin, xmin+t, ymin, ymax, zmin, zmax), surf_id=surf_id)
    sim.add(Obstruction(ion(xb=Bounds3D.of(xmax-t, xmax, ymin, ymax, zmin, zmax), surf_id=surf_id)
    sim.add(Obstruction(ion(xb=Bounds3D.of(xmin, xmax, ymin, ymin+t, zmin, zmax), surf_id=surf_id)
    sim.add(Obstruction(ion(xb=Bounds3D.of(xmin, xmax, ymax-t, ymax, zmin, zmax), surf_id=surf_id)
...
??? question "Obstruction outside mesh"
    ...
    **Solution**: Check mesh and obstruction bounds
    ...
    # Bad: extends to 5.5
    sim.add(Obstruction(ion(xb=Bounds3D.of(4, 5.5, 0, 5, 0, 2.5))

    # Good: within bounds
    sim.add(Obstruction(ion(xb=Bounds3D.of(4, 5, 0, 5, 0, 2.5))
    ```
...
??? question "Surface ID not found"
    ...
    **Solution**: Define surfaces before use
    ...
    sim.add(Obstruction(ion(xb=Bounds3D.of(0, 1, 0, 1, 0, 1), surf_id='WOOD')

    # Good - define first
    sim.surface(id='WOOD', matl_id='PINE')
    sim.add(Obstruction(ion(xb=Bounds3D.of(0, 1, 0, 1, 0, 1), surf_id='WOOD')
    ```
...
??? question "Overlapping geometry"
    ...
    **Solution**: FDS handles overlaps, but avoid if unintentional
    ...
    sim.add(Obstruction(ion(xb=Bounds3D.of(1, 2, 1, 2, 0, 1))
    sim.add(Obstruction(ion(xb=Bounds3D.of(1.5, 2.5, 1, 2, 0, 1))  # Overlaps first
    ```
...
# Desk
sim.add(Obstruction(ion(xb=Bounds3D.of(1, 2.5, 1, 2, 0, 0.75), surf_id='WOOD')

# Filing cabinet
sim.add(Obstruction(ion(xb=Bounds3D.of(0.5, 1, 5, 5.5, 0, 1.2), surf_id='METAL')
