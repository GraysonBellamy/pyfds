...
```python
sim.obstruction(
    xb=(0, 0.2, 0, 5, 0, 2.5),  # Coordinates
...
```python
sim.obstruction(xb=(1.0, 2.0, 1.0, 2.0, 0.0, 0.5))
```
...
# West wall (low X)
sim.obstruction(xb=(0, wall_thickness, 0, 5, 0, 2.5), surf_id='WALL')

# East wall (high X)
sim.obstruction(xb=(5-wall_thickness, 5, 0, 5, 0, 2.5), surf_id='WALL')

# South wall (low Y)
sim.obstruction(xb=(0, 5, 0, wall_thickness, 0, 2.5), surf_id='WALL')

# North wall (high Y)
sim.obstruction(xb=(0, 5, 5-wall_thickness, 5, 0, 2.5), surf_id='WALL')
```
...
sim.obstruction(
    xb=(5-wall_thickness, 5, 0, door_center_y - door_width/2, 0, 2.5),
    surf_id='WALL'
)

# Right part of wall (above door center + half width)
sim.obstruction(
    xb=(5-wall_thickness, 5, door_center_y + door_width/2, 5, 0, 2.5),
    surf_id='WALL'
)

# Top part of wall (above door)
sim.obstruction(
    xb=(5-wall_thickness, 5, door_center_y - door_width/2, door_center_y + door_width/2, door_height, 2.5),
    surf_id='WALL'
)
```
...
sim.obstruction(xb=(0, 5, 0, 5, 0, 0.01), surf_id='FLOOR')

# Ceiling (at top of domain)
sim.obstruction(xb=(0, 5, 0, 5, 2.49, 2.5), surf_id='CEILING')
```
...
sim.obstruction(xb=(2.0, 3.5, 1.5, 2.3, 0.0, 0.8), surf_id='WOOD')

# Chair: 0.5m × 0.5m × 0.5m
sim.obstruction(xb=(1.5, 2.0, 1.5, 2.0, 0.0, 0.5), surf_id='PLASTIC')

# Cabinet: 1m × 0.5m × 2m tall
sim.obstruction(xb=(0.2, 1.2, 4.5, 5.0, 0.0, 2.0), surf_id='METAL')
```
...
sim.obstruction(xb=(2.35, 2.65, 2.35, 2.65, 0.0, 2.5), surf_id='CONCRETE')

# Multiple columns in a row
for x in [1.0, 2.0, 3.0, 4.0]:
    sim.obstruction(xb=(x-0.15, x+0.15, 2.35, 2.65, 0.0, 2.5), surf_id='CONCRETE')
```
...
def create_bookshelf(sim, x, y, z, width, depth, height, shelves):
    ...
    # Back and sides
    sim.obstruction(xb=(x, x+width, y+depth-0.02, y+depth, z, z+height), surf_id='WOOD')
    sim.obstruction(xb=(x, x+0.02, y, y+depth, z, z+height), surf_id='WOOD')
    sim.obstruction(xb=(x+width-0.02, x+width, y, y+depth, z, z+height), surf_id='WOOD')
    ...
    for i in range(shelves + 1):
        ...
        sim.obstruction(
            xb=(x, x+width, y, y+depth, shelf_z, shelf_z+shelf_thickness),
            surf_id='WOOD'
        )
...
for i in range(5):
    for j in range(5):
        ...
        y0 = 0.5 + j * spacing
        sim.obstruction(
            xb=(x0, x0+cube_size, y0, y0+cube_size, 0, cube_size),
            surf_id='BOX'
        )
```
...
for i in range(10):
    ...
    z0 = i * step_rise
    sim.obstruction(
        xb=(x0, x0+step_width, 1.0, 2.0, 0, z0+step_rise),
        surf_id='CONCRETE'
    )
```
...
sim.obstruction(xb=(0, 1, 0, 1, 0, 1))

# Explicit surface
sim.obstruction(xb=(0, 1, 0, 1, 0, 1), surf_id='CONCRETE')
```
...
```python
# Create wall
sim.obstruction(xb=(5, 5.2, 0, 5, 0, 3), surf_id='WALL')

# Cut window in wall
sim.obstruction(xb=(5, 5.2, 1.5, 2.5, 1.5, 2.5), hole=True)
```
...
sim.obstruction(xb=(1.07, 2.13, 1.05, 1.98, 0.02, 0.47))
```
...
```python
# Bad - tiny 0.01m gap
sim.obstruction(xb=(0, 2.49, 0, 5, 0, 2.5))
sim.obstruction(xb=(2.51, 5, 0, 5, 0, 2.5))

# Good - touching or overlapping
sim.obstruction(xb=(0, 2.5, 0, 5, 0, 2.5))
sim.obstruction(xb=(2.5, 5, 0, 5, 0, 2.5))
```
...
sim.obstruction(xb=(4.8, 5, 0, 5, 0, 2.5), surf_id='WALL')

# ============================================================
# FURNITURE
# ============================================================
sim.obstruction(xb=(1, 2, 1, 2, 0, 0.8), surf_id='TABLE')
sim.obstruction(xb=(1, 1.5, 1, 1.5, 0, 0.5), surf_id='CHAIR')

# ============================================================
# FIRE SOURCES
# ============================================================
sim.obstruction(xb=(2.5, 3.5, 2.5, 3.5, 0, 0.1), surf_id='BURNER')
```
...
def add_rectangular_room(sim, xb, wall_thickness, surf_id='WALL'):
    ...
    # Four walls
    sim.obstruction(xb=(xmin, xmin+t, ymin, ymax, zmin, zmax), surf_id=surf_id)
    sim.obstruction(xb=(xmax-t, xmax, ymin, ymax, zmin, zmax), surf_id=surf_id)
    sim.obstruction(xb=(xmin, xmax, ymin, ymin+t, zmin, zmax), surf_id=surf_id)
    sim.obstruction(xb=(xmin, xmax, ymax-t, ymax, zmin, zmax), surf_id=surf_id)
...
??? question "Obstruction outside mesh"
    ...
    **Solution**: Check mesh and obstruction bounds
    ...
    # Bad: extends to 5.5
    sim.obstruction(xb=(4, 5.5, 0, 5, 0, 2.5))
...
??? question "Surface ID not found"
    ...
    **Solution**: Define surfaces before use
    ...
    sim.obstruction(xb=(0, 1, 0, 1, 0, 1), surf_id='WOOD')
...
??? question "Overlapping geometry"
    ...
    **Solution**: FDS handles overlaps, but avoid if unintentional
    ...
    sim.obstruction(xb=(1, 2, 1, 2, 0, 1))
    sim.obstruction(xb=(1.5, 2.5, 1, 2, 0, 1))  # Overlaps first
    ```
...
# Desk
sim.obstruction(xb=(1, 2.5, 1, 2, 0, 0.75), surf_id='WOOD')

# Filing cabinet
sim.obstruction(xb=(0.5, 1, 5, 5.5, 0, 1.2), surf_id='METAL')
